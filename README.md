...
- **本机用量索引** — Rust 已能只读扫描 Codex／Claude Code 的 JSONL，按 Token、模型、速度和对话聚合，并按公开 API 目录估算等值费用；当前还没有 Vue 展示入口。
- **中英文、深浅色** — 都可以跟随系统。
- **可开机自动启动** — 设置里一个开关。
...
**它会显示我花了多少钱、聊了多少次吗？**
Rust 数据层已经能统计 Token、对话元数据和 API 等值费用，但当前版本尚未实现对应 Vue 页面。这里的费用只是相同 Token 按公开 API 标价的估算，不是 Codex／Claude 订阅实付、账单或发票。
...
`0.1.0` 开发中，尚未正式发布。目前已完成桌面壳、状态模型、刷新调度、设置持久化和本地用量 Rust 数据切片，Codex 与 Claude Code 的真实额度闭环已在 macOS 跑通一次；用量 Vue 页面未实现，**Windows 侧尚未实机验证**，功能与体验都可能继续调整。
