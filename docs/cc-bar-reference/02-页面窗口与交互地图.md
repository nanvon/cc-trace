...
- Service：All、Codex、Claude Code。Antigravity 不在本地用量统计过滤器中。**代码已确认**。
...
- 顶部 KPI：总 Tokens、总花费、Codex、Claude Code，并可显示与上一等长区间的 delta。**代码已确认**。
- Token breakdown：输入、输出、缓存命中、命中率；Fast 时追加原始 Tokens、计费等效 Tokens、倍率、估算费用和占比。**代码已确认**。
- By service：按费用占比、总 Token、Token 分类、Fast 摘要。**代码已确认**。
- Daily usage：Codex/Claude 堆叠柱图；单日范围时会扩展近 14 天上下文，支持 hover 选中日和明细 tooltip。**代码已确认**。
- By model：Codex / Claude 分组，显示模型 Token、费用、Token 拆分、Fast。**代码已确认**。
...
实际分组：Accounts、Other Codex Accounts、Menu Bar、Floating HUD、Refresh、General。包括 Provider 全局/菜单栏/HUD 开关、菜单栏周期、HUD 开关、额度/日志间隔、reset time、状态圆点、重新计算、语言、privacy mode、launch at login、版本。**代码已确认**。
