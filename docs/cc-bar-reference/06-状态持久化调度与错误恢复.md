...
**代码已确认**：实际 Settings 没有 `menuBarVariant`、`floatingVariant`、`floatingPosition`、`floatingIdleOpacity`、`autoRefreshEnabled`、`showInDock` 等字段（cc-bar 文档已随 2026-08-08 `f595115` 删除这些未实现项的描述，冲突解除）。
...
| scan state | `scan-state.json` v11 | 文件 mtime/size/offset、Codex model/tier/signature、Claude/Pi/OpenCode 跨文件 seen 集与 watermark、conversation metadata | 路径、会话 ID、标题 | 可由外部 JSONL 重建 | version/fingerprint/generation 无效则 invalidate |
