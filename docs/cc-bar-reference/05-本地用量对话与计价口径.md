...
`PricingCatalogStore`（2026-07-30 `2cea531` 后）：

- 本地 `pricing-catalog.json` v2，LiteLLM/models.dev 各自保存 ETag、成功／失败时刻、
  Standard、Codex Fast、Claude Fast，并持久化缺价刷新冷却；
- models.dev 读取 `experimental.modes.fast.cost`；LiteLLM 读取 OpenAI `*_priority`；
- 正常约 24 小时刷新，失败和同一缺价键约 30 分钟重试；支持设置页手动强制更新；
- 网络结果先进入 pending，在扫描安全边界 commit；
- Standard 可疑大幅 shrink 保留旧目录，Fast 使用 last-known 合并；
- fingerprint 只纳入本机实际出现过的 app/model/speed 远端价格，并包含本地特殊规则与倍率。

**代码已确认**：动态目录改变 fingerprint 后会使 rollup/scan state 失效并重算。网络真实可用性、本地目录当前内容和第三方价格的正确性是**待确认**，本次未联网。

### OpenCode 与 Pi 的价格短路（2026-08-07 `49ad45c`）

- `PricingCatalogStore.rate` 对 `app == .pi`、`app == .opencode` 的 Standard 与 Fast 都返回 nil：**不查在线价格目录**。
- `Pricing.needsRemotePriceRefresh` 对 pi／opencode 恒 false：缺价**不触发远端刷新**。
- 两者都不参与 fingerprint 的远端合并价部分（rate 返回 nil 被过滤）；本地静态表 `baseBody` 仍全局参与。
- 分项差异：pi 完全不走任何价格表（`price(for:)` 恒 nil）；**opencode 的分项补算只走本地静态表**（如 OpenCode 里用到已收录的 Claude 系模型时能拆分），本地表未收录时官方 `cost` 总额降级计入 output，保证对话页与统计页总额一致且不误标「未定价」。**代码已确认**。

## UI 消费口径

| UI | 数据 | 口径 |
|---|---|---|
| Popover 服务卡 | AppState quota snapshot + UsageAggregator 今日/本周 | Codex/Claude 显示估算花费；Antigravity 不显示本地 cost；Pi/OpenCode 有 `piTodayCost`／`opencodeTodayCost` 字段但 Popover 不展示 |
| Overview KPI | UsageAggregator | 当前日期范围总 Token/费用，与上一等长范围 delta |
| Daily chart | 按 day 合并可见服务（Codex/Claude/Pi/OpenCode） | 柱高按当日各服务 `totalTokens` 堆叠（输入含缓存读/写 + 输出），2026-08-03 `a22fa34` 起不再按费用；单日选择会追加近 14 天上下文，但 KPI 仍只算选定范围 |
| By service/model | UsageAggregator | Token、input/output/cache hit、命中率、Fast 信息和费用；占比分母只统计设置页「统计服务」中可见的服务 |
