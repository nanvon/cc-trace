...
语义刷新并把结果原子回写同一来源，Vue 只接收额度快照、用量聚合、扫描状态和展示状态（额度
身份含完整账号，边界见 docs/决策/ADR-0025）。它拥有独立于 Swift 版 cc-bar 的应用身份、数据目录、缓存和发布体系，不读取、迁移、
覆盖或删除 cc-bar 的应用数据。
...
- 价格目录由随应用升级的内置政策与 LiteLLM／models.dev 在线 Standard／Fast 缓存合并；
  费用是按模型、速度和推理地域估算的 API 等值金额，不是订阅实付金额，未知价格返回未定价。
- Fast 保留原始 Token，并另行提供计费等效 Token 与可核验倍率；无法安全确定时不猜。
...
- 基础设置包括语言、外观、自动刷新间隔、开机启动、手动更新价格目录和版本信息。
