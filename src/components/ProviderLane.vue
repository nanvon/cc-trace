...
const primaryParts = computed(() =>
  primaryHasValue.value && primary.value?.remainingPercent !== null
    ? splitPercent(locale.value, primary.value.remainingPercent)
    : { value: t("quota.noValue"), unit: "" },
);
const primaryFill = computed(() => {
  const percent = primary.value?.remainingPercent;
  return primaryHasValue.value && percent !== null
...
const primaryValueA11y = computed(() =>
  primaryHasValue.value && primary.value?.remainingPercent !== null
    ? t("a11y.remaining", {
        percent: formatPercent(locale.value, primary.value.remainingPercent),
      })
    : t("a11y.noQuota"),
);
  ...
  <article class="lane" :class="[`lane--${variant}`, `lane--${presentation.tone}`]">
    ...
    <template v-if="variant === 'compact'">
      ...
      <template v-if="showsRails">
        ...
        >
          <span v-if="primaryHasValue" class="lane-bar__fill" :style="{ inlineSize: `${primaryFill}%` }" />
        </div>
